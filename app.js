...
app.get('/loginCal/:userID', function (req, resp) {
    ...
    if (obj3 != undefined) {
        for (i = 0; i < obj3.length; i++) {
            if (userid == obj3[i].vol.username || userid == obj3[i].dog.username) {
                found2 = true
                let Fname = obj3[i].vol.name.split(" ");
                walk_days.push({
                    day: obj3[i].day,
    ...
    if (obj3 == undefined || found === false || found2 === false) {
        resp.sendStatus(404);
    } else {
        //console.log("USER: ", userid)
...
app.post('/addVol', function (req, resp) {
    ...
    let b=JSON.parse(a);
    //console.log('b',b);
    //console.log('b_access: ', b.access_token);
    
    //var d = JSON.parse(b.dedV)
   // console.log('d:',d)
   // console.log('1: ', JSON.parse(a).access_token);
    //console.log('2: ', d[0].access_token);
    //console.log('3: ', JSON.parse(a).access_token);
    //var days = b.dedV.daysV;
    //var days_arr = days.split(',');
 // console.log("access to",d[0].access_token)
    if(b.access_token == ''){
        
        resp.sendStatus(404)
    }else{
        let c=JSON.parse(b.dedV)
    //console.log(c[0].daysV);
